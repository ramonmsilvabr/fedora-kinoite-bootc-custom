...
* `fedora-kinoite-bootc-custom` que **não** inclui os drivers proprietários da NVIDIA; 
* `fedora-kinoite-bootc-custom-nvidia-open` que inclui os drivers proprietários sem as dependências de compilação.

As duas imagens são baseadas no Fedora Kinoite, isso significa que o ambiente Desktop de escolha é o KDE na sua versão mais recente.
...
* Se você já estiver em qualquer edição atômica do Fedora ou derivados, você pode puxar a imagem direto do registro. Note que não tem suporte a Secure Boot.
    ...
    * Edição com drivers da NVIDIA:
    ```
    sudo bootc switch ghcr.io/ramonmsilvabr/fedora-kinoite-bootc-custom-nvidia-open:latest
    ```

    * Edição sem drivers da NVIDIA:
    ```
    sudo bootc switch ghcr.io/ramonmsilvabr/fedora-kinoite-bootc-custom:latest
    ```